...
import { UserCircle2, Mail, UserPlus, Users } from 'lucide-react'; // Added Users icon

export default function CreatorsPage() {
  return (
    <div className="space-y-8">
      <header className="text-center animate-fade-in-up">
        <Users className="w-12 h-12 text-primary mx-auto mb-3" /> {/* Updated icon */}
            ...
            >
              <CardHeader className="items-center text-center">
                <div className="relative w-32 h-32 rounded-full overflow-hidden border-4 border-accent mb-4 group-hover:scale-105 transition-transform">
                  <Image
                    src={creator.photoUrl || `https://placehold.co/128x128.png?text=${encodeURIComponent(creator.name.split(' ')[0])}`}
                    alt={creator.name}
                    width={128}
                    height={128}
      ...
      <Card className="shadow-lg animate-fade-in-up bg-gradient-to-br from-primary/10 to-accent/10" style={{ animationDelay: `${0.2 + (MOCK_CREATORS.length > 0 ? MOCK_CREATORS.length * 0.1 : 0.1)}s` }}>
        <CardHeader className="items-center text-center">
